...
#' @param t1 numeric vector. See Value.
#' @param t2 numeric vector. See Value.
#' @param s2 numeric vector. See Value.
#' @param t1equal logical value. See \strong{csurv} in Value.
...
predjsurv<- function(data,fit,t1.formula, t2.formula,t1, t2,method=c("np","sp"),newdata){
  ...
  if(method=="np"){
    ...
    if(indx[1]!=0){
      ...
      data<- data$data
      t1data <-  prepData(formula=t1.formula, data, na.action)
      t2data <-  prepData(formula=t2.formula, data, na.action)
