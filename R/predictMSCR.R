#' @noRd
predictMSCR<- function(fit, t1obs,times,type=c("surv", "rrms", "rmst", "qrl","qst"),tu,tau=0.5, nsim=1000){
...
#' @noRd
dypredfit<- function(fit,t1obs,type,times,nsim,tu,tau,exact=FALSE){
  ## type = "surv": survival function
  ## type = "rrms": restricted residual mean survival
  ## type = "rmst": restricted residual mean survival+ landmark time rrms+max(t1obs)
  ## type = "qrl": quantile residual lifetime
  ...
  if(lpos==1){
    cs<- predcsurv(fit=fit$mar.fits[[t1pos]],t1=t1obs, t2=times, s2=t1obs,
                   t1equal =TRUE,method="sp")$csurv[[1]][,1]
    if(type=="surv"){
  ...
  if(lpos>1){
    ...
    Qstep<- dypred.Q(fit = fit,t1obs = t1obs,t1pos = t1pos,nsim=nsim,exact = exact)
  ...
  if(type%in%c("qrl", "qst")){
    ...
    if(length(cs)<=2){
      ...
      qrl<- approx(x = cs,y=tjp,xout = tau,method = "constant",ties = max,f=1)$y
    }
...
#' @noRd
dypred.Q<- function(fit,t1obs,t1pos,nsim=500, exact=FALSE){
  jps<- knots(fit$S_D)
  ...
  Sdev_k<- lapply(1:lpos,function(k)
    survnumdiff(fit$mar.fits[[t1pos[k]]]$t1.surv$time,
                fit$mar.fits[[t1pos[k]]]$t1.surv$surv, exact=exact))
